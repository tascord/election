...
use crate::ty::{
    FirstPreferences, PartyAffilliation, PrefDistribution, TcpPP, TwoCandidatePreferred,
};
    ...
    fn process(data: &[u8]) -> anyhow::Result<Vec<Self>> {
        ...
        Ok(data
            ...
            .map(|c| -> anyhow::Result<PrefDistribution> {
                let party = c[0].get("PartyAb").ok_or(anyhow!("No field 'PartyAb'"))?;
                let party = PartyAffilliation::from_str(party)?;
            ...
            .filter_map(|v| v.inspect_err(|e| warn!("Broken row: {e:?}")).ok())
            .collect())
    }
}
        ...
        Ok(data
            ...
            .map(|c| -> anyhow::Result<TwoCandidatePreferred> {
                ...
                let parties = c.iter().map(|p| -> anyhow::Result<TcpPP> {
                    ...
                    let swing: f32 = p.get("Swing").ok_or(anyhow!("No field 'Swing'"))?.parse()?;
...
fn csv(data: &[u8]) -> anyhow::Result<Vec<HashMap<String, String>>> {
    ...
    let data = lines.par_bridge().into_par_iter().map(|v| {
        let mut values = Vec::new();
        let mut escape = false;
        let mut current = String::new();
        for c in v.chars() {
